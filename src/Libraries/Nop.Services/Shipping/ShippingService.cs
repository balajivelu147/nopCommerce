...
namespace Nop.Services.Shipping
{
    ...
    public partial class ShippingService : IShippingService
    {
        ...
        /// <returns>
        /// A task that represents the asynchronous operation
        /// The task result contains the warehouses of a vendor
        /// </returns>
        public virtual async Task<IList<Warehouse>> GetAllWarehousesByVendorAsync(int vendorId)
        {
            var warehouses = await _warehouseRepository.GetAllAsync(query =>
            {
                return from wh in query
        ...
        public virtual async Task<dynamic> GetAllAddressOfWarehouseByVendorAsync(int vendorId)
        {

            var warehousesOfVendor = await GetAllWarehousesByVendorAsync(vendorId);
            var addressOfWarehouse = await _addressService.GetAddressesByIdsAsync(warehousesOfVendor.Select(x => x.AddressId).ToList());

            //TODO: should remove class functions, instead use tuples[WarehouseWithLatLong{l1.Id, l1.Name, l2.Latitude, l2.Longitude }]
            //(https://stackoverflow.com/questions/5013472/what-is-the-return-type-of-my-linq-query)
            //should remove dynamic also
            return (from l1 in warehousesOfVendor
                         join l2 in addressOfWarehouse
                         on l1.AddressId equals l2.Id
                         select new WarehouseWithLatLong(l1.Id, l1.Name, l2.Latitude, l2.Longitude )).ToList();
        }
